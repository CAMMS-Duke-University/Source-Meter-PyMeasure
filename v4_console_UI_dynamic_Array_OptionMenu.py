...
class App(customtkinter.CTk):
    ...
    def generate_group_frame(self):
        ...
        for i in range(0, int(self.status_label_value.cget("text"))):
            ...
            single_frame = self.generate_single_frame('Instrument '+str(i+1))
            self.group_data_tk.append(single_frame)
        self.enty_button = customtkinter.CTkButton(master=self.group_frame, command=self.update_data, text="START")
        self.enty_button.grid(row=1, column=2, pady=10, padx=20, sticky="n")

    # ----------------------------------This Generates the GPIB Group Frames--------------------------------------------
    def generate_single_frame(self, single_frame_head):
        # Form Head
        self.single_frame_title = customtkinter.CTkLabel(self.single_frame, text=single_frame_head, font=("Calibri", 16))
    ...
    def update_data(self):
        for single_group_data_tk in self.group_data_tk:
            print(len(single_group_data_tk),"-----------------")
            mode =  single_group_data_tk[0].get()
            print("(Mode)---", mode)
            label = single_group_data_tk[1].cget("text")
            entry = single_group_data_tk[2].get()
            print("---(Lable 1)---", label, "-->", entry)
            label = single_group_data_tk[3].cget("text")
            entry = single_group_data_tk[4].get()
