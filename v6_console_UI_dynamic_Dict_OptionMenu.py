...
class App(customtkinter.CTk):
    ...
    def generate_single_frame(self):
        ...
        single_frame_mode_values = ["Apply Steady Voltage",
                                    "Apply Incremental Voltage"]
        ...
        for label, entry in self.single_frame_data.items():
            if label != "Instrument" and label != "OptionMenu":
                if label == "Mode":
                    self.single_frame_mode_text = customtkinter.CTkLabel(self.single_frame_form, text=label)
                    self.single_frame_mode_text.grid(row=row_counter, column=0, sticky=tkinter.W, pady=2, padx=(5, 5))
                    self.single_frame_mode_val = customtkinter.CTkLabel(self.single_frame_form, text=entry)
                    self.single_frame_mode_val.grid(row=row_counter, column=1, pady=2, padx=(5, 5))
                    single_frame_tuple.append(("Mode", self.single_frame_mode_val))
                else:
    ...
    def sidebar_option_menu_event(self, new_appearance_mode: str):
        ...
        for i in range(0, int(self.status_label_value.cget("text"))):
            self.group_data.append({
                "Instrument": str(i + 1),
                "OptionMenu": "Apply Steady Voltage",
    ...
    def update_data_event(self):
        ...
        print("Length:", len(self.group_data))
        for single_group_data_tk in self.group_data_tk:
            single_group_data = dict(Instrument=single_group_data_tk[0][1].cget("text"),
                                     ...
                                     Mode=single_group_data_tk[2][1].cget("text"))
            for i in range(3, len(single_group_data_tk)):
            ...
            print("Before OptionMenu:", self.group_data[0]["OptionMenu"])
            print("New OptionMenu:", updated_group_data[0]["OptionMenu"])
